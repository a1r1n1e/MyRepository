...
import whobuys.vovch.vovch.light.data_layer.async_tasks.LogouterTask;
...
public class UserSessionData {
    ...
    private static final String APP_PREFERENCES_GROUP_ID = "group_id";
    ...
    private static final String DEFAULT_NOT_EXISTING_GROUP_VALUE = "-100";
        ...
        try {
            ...
            if (context != null) {
                ...
                if (preferences != null) {
                    ...
                    groupId = preferences.getString(APP_PREFERENCES_GROUP_ID, DEFAULT_NOT_EXISTING_GROUP_VALUE);
                }
            }
        } catch (Exception e){
            Log.d("WhoBuys", "USD");
    ...
    public void checkUserData(String newId, String newLogin, String newPassword, String newSession, String userGroupId) {
        try {
            id = newId;
            login = newLogin;
            password = newPassword;
            ...
            savePrefs(newId, newLogin, newPassword, newSession, userGroupId);
        } catch (Exception e){
            Log.d("WhoBuys", "USD");
        }
    }
    ...
    public boolean isAnyPrefsData() {
        ...
        try {
            if (preferences != null &&
                    preferences.getString(APP_PREFERENCES_GROUP_ID, null) != null &&
    ...
    public void savePrefs(final String userId, final String userName, final String userPassword, final String userSession, final String userGroupId) {    // Should be used not from UI thread
        try {
            SharedPreferences.Editor editor = preferences.edit();
            ...
            editor.putString(APP_PREFERENCES_GROUP_ID, userGroupId);
            editor.apply();
        } catch (Exception e){
            Log.d("WhoBuys", "USD");
        }
    ...
    public void showExitGood(){
        try {
            ...
            password = null;
            session = DEFAULT_NOT_EXISTING_SESSION_VALUE;
            savePrefs(null, null, null, null, null);
            if (context != null) {
        ...
        try {
            ...
            if(tempResult != null && tempResult.length() > 2) {
                ...
                if (prefixString.equals("200")) {
                    ...
                    String groupId = WebCall.getStringFromJsonString(postfixString, "group_id");
                    setSession(sessionValue);
                    checkUserData(newId, newLogin, newPassword, sessionValue, groupId);
    ...
    public String registration(String newLogin, String newPassword){            //not used in this version
        try {
            ...
            if (token != null && newLogin != null && newPassword != null) {
                ...
                if (tempResult != null && tempResult.length() > 2) {
                    ...
                    if (prefixString.equals("200")) {
                        ...
                        String newId = webCall.getStringFromJsonString(postfixString, "id");
                        checkUserData(newId, newLogin, newPassword, newSessionId, groupId);
    ...
    public void register(String phoneNumber){                           //TODO total personal data shit
        try {
            StringBuilder loginResult = null;
            String tempResult;
            WebCall webCall = new WebCall();
            ...
            if (token != null && phoneNumber != null) {
                tempResult = webCall.callServer(phoneNumber, phoneNumber, DEFAULT_NOT_EXISTING_SESSION_VALUE, "registration", jsonString, UserSessionData.this);
                if (tempResult != null && tempResult.length() > 2) {
                    String prefixString = tempResult.substring(0, 3);
                    String postfixString = tempResult.substring(3);
                    loginResult = new StringBuilder(prefixString);
                    if (prefixString.equals("200")) {
                        ...
                        String newId = WebCall.getStringFromJsonString(postfixString, "id");
                        String groupId = WebCall.getStringFromJsonString(postfixString, "group_id");
                        checkUserData(newId, phoneNumber, phoneNumber, newSessionId, groupId);
                        loginResult.append(newId);
                    }
                }
            }
        } catch (Exception e){
            Log.d("auchan_test", "registration");
        }
    }
